...
Experiments::ThreeSampleNaive50kUCTAgentExperiment::ThreeSampleNaive50kUCTAgentExperiment(){
...
void Experiments::ThreeSampleNaive50kUCTAgentExperiment::write_header(){
...
void Experiments::ThreeSampleNaive50kUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::ThreeSampleNaive50kUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::ThreeSampleNaive50kUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::ThreeSampleNaive50kUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::ThreeSampleNaive50kUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 50000 simulations per step
    // 1 determinization per stochasticity
    return new Agents::KSampleNaiveUCTAgent(*game,50000,3);
}

std::vector<Measurements::GameMeasurement*> Experiments::ThreeSampleNaive50kUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::ThreeSampleNaive50kUCTAgentExperiment();
