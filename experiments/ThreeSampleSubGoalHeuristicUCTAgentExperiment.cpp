...
Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::ThreeSampleSubGoalHeuristicUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "ThreeSampleSubGoalHeuristicUCTAgentExperiment";
    ...
    fileheader = "ThreeSampleSubGoalHeuristicUCTAgentExperiment";// .header ,.csv
    
    // Use the scenario to setup some variables
    scenario = new Scenarios::VanillaGameScenario();

    agent_name = "Three-Sample SubGoal Heuristic UCT Agent";
...
void Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::write_header(){
...
void Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 3 determinization per stochasticity
    return new Agents::KSampleSubGoalHeuristicUCTAgent(*game,10000,3);
}

std::vector<Measurements::GameMeasurement*> Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::ThreeSampleSubGoalHeuristicUCTAgentExperiment();
