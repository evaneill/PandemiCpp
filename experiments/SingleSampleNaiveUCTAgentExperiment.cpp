...
#include "../agents/SingleSampleNaiveUCTAgent.h"
...
Experiments::SingleSampleNaiveUCTAgentExperiment::SingleSampleNaiveUCTAgentExperiment(){
    ...
    description = "Test a single-stochastic-sample UCT agent that uses only win/loss (0/1) reward from rollouts to update node scores, and 1000 simulations/step";
...
Agents::BaseAgent* Experiments::SingleSampleNaiveUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 1000 simulations per step
    return new Agents::SingleSampleNaiveUCTAgent(*game,1000);
}
