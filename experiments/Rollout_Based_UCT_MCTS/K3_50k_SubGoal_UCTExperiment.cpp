...
#include "../../agents/Rollout_Based_UCT_MCTS/KSample_Subgoal_UCTAgent.h"

#include "../../experimental_tools/Experiments.h"
#include "../../experimental_tools/Scenarios.h"
#include "../../experimental_tools/Measurements.h"

Experiments::K3_50k_SubGoal_UCTExperiment::K3_50k_SubGoal_UCTExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "K3_50k_SubGoal_UCTExperiment";
    ...
    fileheader = "K3_50k_SubGoal_UCTExperiment";// .header ,.csv
...
void Experiments::K3_50k_SubGoal_UCTExperiment::write_header(){
...
void Experiments::K3_50k_SubGoal_UCTExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::K3_50k_SubGoal_UCTExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::K3_50k_SubGoal_UCTExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::K3_50k_SubGoal_UCTExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::K3_50k_SubGoal_UCTExperiment::get_agent(GameLogic::Game* game){
    // 50000 simulations per step
    // 3 determinization per stochasticity
    return new Agents::KSample_Subgoal_UCTAgent(*game,50000,3);
}

std::vector<Measurements::GameMeasurement*> Experiments::K3_50k_SubGoal_UCTExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::K3_50k_SubGoal_UCTExperiment();
