...
#include "../../agents/Rollout_Based_UCT_MCTS/KSample_Precondition_UCTMaxChildAgent_SmartRollout.h"

#include "../../experimental_tools/Experiments.h"
#include "../../experimental_tools/Scenarios.h"
#include "../../experimental_tools/Measurements.h"

Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment";
    description = "Test a single-determinization UCT agent that uses heavy epsilon-greedy rollouts (eps = .1) w/ 'Smart Compound' heuristic and cure precondition evaluation of terminal rollout states";

    fileheader = "K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment";// .header ,.csv
    
    // Use the scenario to setup some variables
    scenario = new Scenarios::VanillaGameScenario();

    agent_name = "Single-Sample Heavy-rollout agent w/ epsilon-greedy compound heuristic hill-climbing rollouts and cure precondition rollout evaluations";
...
void Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::write_header(){
...
void Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 1 determinization per stochasticity
    // epsilon = .1
    // convergence at 1 visit (treat value as known with certainty)
    return new Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout(*game,500,1,.1,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::K1_500_Precondition_UCTMaxChild_SmartRolloutExperiment();
