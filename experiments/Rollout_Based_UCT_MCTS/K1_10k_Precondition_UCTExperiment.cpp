...
Experiments::K1_10k_Precondition_UCTExperiment::K1_10k_Precondition_UCTExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "K1_10k_Precondition_UCTExperiment";
    description = "Test a single-determinization UCT agent that uses (# diseases cured /4)+.15*SUM(max fraction satisfied preconditions for uncured disease i among players) + research proximity reward from rollouts to update node scores";

    fileheader = "K1_10k_Precondition_UCTExperiment";// .header ,.csv
...
void Experiments::K1_10k_Precondition_UCTExperiment::write_header(){
...
void Experiments::K1_10k_Precondition_UCTExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::K1_10k_Precondition_UCTExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::K1_10k_Precondition_UCTExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::K1_10k_Precondition_UCTExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::K1_10k_Precondition_UCTExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 1 determinization per stochasticity
    return new Agents::KSample_Precondition_UCTAgent(*game,10000,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::K1_10k_Precondition_UCTExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::K1_10k_Precondition_UCTExperiment();
