...
Experiments::K3_50k_Precondition_UCTMaxChildExperiment::K3_50k_Precondition_UCTMaxChildExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "K3_50k_Precondition_UCTMaxChildExperiment";
    description = "Test a three-determinization UCT agent that uses cure precondition heuristic reward from rollouts to update node scores, and uses max-child to select an action";

    fileheader = "K3_50k_Precondition_UCTMaxChildExperiment";// .header ,.csv
...
void Experiments::K3_50k_Precondition_UCTMaxChildExperiment::write_header(){
...
void Experiments::K3_50k_Precondition_UCTMaxChildExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::K3_50k_Precondition_UCTMaxChildExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::K3_50k_Precondition_UCTMaxChildExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::K3_50k_Precondition_UCTMaxChildExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::K3_50k_Precondition_UCTMaxChildExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 3 determinization per stochasticity
    // Will take max-avg-reward children if >=1 visits (this is to match heuristic-evaluated agents)
    return new Agents::KSample_Precondition_UCTMaxChildAgent(*game,10000,3,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::K3_50k_Precondition_UCTMaxChildExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::K3_50k_Precondition_UCTMaxChildExperiment();
