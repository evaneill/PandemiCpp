...
Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::ThreeSampleGoalHeuristic50kUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "ThreeSampleGoalHeuristic50kUCTAgentExperiment";
    description = "Test a three-determinization UCT agent that uses only (# diseases cured /4) reward from rollouts to update node scores (50k simulations/step)";

    fileheader = "ThreeSampleGoalHeuristic50kUCTAgentExperiment";// .header ,.csv
    
    // Use the scenario to setup some variables
    scenario = new Scenarios::VanillaGameScenario();

    agent_name = "Three-Sample Goal Heuristic UCT Agent";
...
void Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::write_header(){
...
void Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 50000 simulations per step
    // 3 determinization per stochasticity
    return new Agents::KSampleGoalHeuristicUCTAgent(*game,50000,3);
}

std::vector<Measurements::GameMeasurement*> Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::ThreeSampleGoalHeuristic50kUCTAgentExperiment();
