...
Experiments::SingleSampleNaive50kUCTAgentExperiment::SingleSampleNaive50kUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "SingleSampleNaive50kUCTAgentExperiment";
    ...
    fileheader = "SingleSampleNaive50kUCTAgentExperiment";// .header ,.csv
...
void Experiments::SingleSampleNaive50kUCTAgentExperiment::write_header(){
...
void Experiments::SingleSampleNaive50kUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::SingleSampleNaive50kUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::SingleSampleNaive50kUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::SingleSampleNaive50kUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::SingleSampleNaive50kUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 50000 simulations per step
    // 1 determinization per stochasticity
    return new Agents::KSampleNaiveUCTAgent(*game,50000,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::SingleSampleNaive50kUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::SingleSampleNaive50kUCTAgentExperiment();
