...
#include "../agents/KSampleSubGoalHeuristicUCTMaxChildAgent.h"

#include "../experimental_tools/Experiments.h"
#include "../experimental_tools/Scenarios.h"
#include "../experimental_tools/Measurements.h"

Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::K3_10k_SubGoal_UCTMaxChildExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "K3_10k_SubGoal_UCTMaxChildExperiment";
    ...
    fileheader = "K3_10k_SubGoal_UCTMaxChildExperiment";// .header ,.csv
...
void Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::write_header(){
...
void Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 3 determinization per stochasticity
    // Will take max-avg-reward children if >=100 visits 
    return new Agents::KSampleSubGoalHeuristicUCTMaxChildAgent(*game,10000,3,100);
}

std::vector<Measurements::GameMeasurement*> Experiments::K3_10k_SubGoal_UCTMaxChildExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::K3_10k_SubGoal_UCTMaxChildExperiment();
