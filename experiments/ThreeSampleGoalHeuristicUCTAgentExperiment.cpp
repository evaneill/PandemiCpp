...
Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::ThreeSampleGoalHeuristicUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "ThreeSampleGoalHeuristicUCTAgentExperiment";
    description = "Test a three-determinization UCT agent that uses only (# diseases cured /4) reward from rollouts to update node scores";

    fileheader = "ThreeSampleGoalHeuristicUCTAgentExperiment";// .header ,.csv
...
void Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::write_header(){
...
void Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 3 determinization per stochasticity
    return new Agents::KSampleGoalHeuristicUCTAgent(*game,10000,3);
}

std::vector<Measurements::GameMeasurement*> Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::ThreeSampleGoalHeuristicUCTAgentExperiment();
