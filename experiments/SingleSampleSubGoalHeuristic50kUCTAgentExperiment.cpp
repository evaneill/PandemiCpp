...
#include "../agents/KSampleSubGoalHeuristicUCTAgent.h"

#include "../experimental_tools/Experiments.h"
#include "../experimental_tools/Scenarios.h"
#include "../experimental_tools/Measurements.h"

Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::SingleSampleSubGoalHeuristic50kUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "SingleSampleSubGoalHeuristic50kUCTAgentExperiment";
    description = "Test a single-determinization UCT agent that uses (# diseases cured /4)+SUM(max fraction satisfied preconditions for uncured disease i among players) reward from rollouts to update node scores (50k sims/step)";

    fileheader = "SingleSampleSubGoalHeuristic50kUCTAgentExperiment";// .header ,.csv
    
    // Use the scenario to setup some variables
    scenario = new Scenarios::VanillaGameScenario();

    agent_name = "Single-Sample SubGoal Heuristic UCT Agent";
...
void Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::write_header(){
...
void Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 50000 simulations per step
    // 1 determinization per stochasticity
    return new Agents::KSampleSubGoalHeuristicUCTAgent(*game,50000,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::SingleSampleSubGoalHeuristic50kUCTAgentExperiment();
