...
Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::SingleSampleSubGoalHeuristicUCTAgentExperiment(){
    // Hard-code a description for this experiment
    experiment_name = "SingleSampleSubGoalHeuristicUCTAgentExperiment";
    description = "Test a single-determinization UCT agent that uses (# diseases cured /4)+SUM(max fraction satisfied preconditions for uncured disease i among players) reward from rollouts to update node scores";

    fileheader = "SingleSampleSubGoalHeuristicUCTAgentExperiment";// .header ,.csv
...
void Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::write_header(){
...
void Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::reset_board(Board::Board* game_board){
    scenario -> reset_board(game_board);
}

void Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::append_header(std::string extras){
    std::ofstream header(Experiments::OUTPUT_DIR + fileheader+".header",std::ios::out | std::ios::app);
    header << extras;
    header.close();
}

void Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::write_experiment(std::string data){
    std::ofstream logfile(Experiments::OUTPUT_DIR + fileheader + ".csv",std::ios::out | std::ios::trunc);
    logfile << data;
    logfile.close();
}

Board::Board* Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::get_board(){
    return scenario -> make_board({1,2,3},4);
}

Agents::BaseAgent* Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::get_agent(GameLogic::Game* game){
    // 10000 simulations per step
    // 1 determinization per stochasticity
    return new Agents::KSampleSubGoalHeuristicUCTAgent(*game,10000,1);
}

std::vector<Measurements::GameMeasurement*> Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment::get_game_measures(Board::Board* game){
...
int main(){
    Experiments::Experiment* experiment = new Experiments::SingleSampleSubGoalHeuristicUCTAgentExperiment();
