...
#include "KSample_Naive_UCTMaxChildAgent.h"

#include "../search_tools/Search.h"

Agents::KSample_Naive_UCTMaxChildAgent::KSample_Naive_UCTMaxChildAgent(GameLogic::Game& _active_game, int _n_simulations, int _K,int _VisitConvergenceCriteria):
...
Actions::Action* Agents::KSample_Naive_UCTMaxChildAgent::generate_action(bool verbose){
    ...
    state_values.push_back(search_tree -> root -> TotalReward / (double) search_tree -> root -> N_visits); // save the "average value" of the root
...
Search::Node* Agents::KSample_Naive_UCTMaxChildAgent::get_max_child(Search::Node* root){
...
double Agents::KSample_Naive_UCTMaxChildAgent::get_max_avgreward(Search::Node* node){
    if(node -> terminal){
...
void Agents::KSample_Naive_UCTMaxChildAgent::take_step(bool verbose){
...
void Agents::KSample_Naive_UCTMaxChildAgent::reset(){
    tree_depths.clear();
...
std::vector<std::string> Agents::KSample_Naive_UCTMaxChildAgent::get_keys(){
    return {
        "AvgTreeDepth",
        "StdTreeDepth",
        "MaxTreeDepth",
...
std::vector<double> Agents::KSample_Naive_UCTMaxChildAgent::get_values(){
    // Avg tree depth
    double depth_mean=0;
