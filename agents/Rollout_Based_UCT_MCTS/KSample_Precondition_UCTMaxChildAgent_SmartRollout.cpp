...
#include "KSample_Precondition_UCTMaxChildAgent_SmartRollout.h"

#include "../search_tools/Search.h"

Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::KSample_Precondition_UCTMaxChildAgent_SmartRollout(GameLogic::Game& _active_game, int _n_simulations, int _K,double _epsilon,int _VisitConvergenceCriteria):
...
Actions::Action* Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::generate_action(bool verbose){
    ...
    while(sims_done<n_simulations){   
        ...
        // Use a epsilon-greedy heavy rollout using compound heuristic that was pretty successful with heuristic-evaluated agents
        // Evaluate terminal states with most detailed CureGoalConditionswStation heuristic
        double reward = active_game.epsgreedy_heuristic_rollout(board_copy, epsilon,
            [](Board::Board& _board){ 
                // selection policy = (2/3) - (1/3) weighted compound heuristic
                return Heuristics::CompoundHeuristic(_board,Heuristics::CureGoalConditionswStation,Heuristics::SmartLossProximity,2./3.);
            },
            Heuristics::CureGoalConditionswStation);
...
Search::Node* Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::get_max_child(Search::Node* root){
...
double Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::get_max_avgreward(Search::Node* node){
    if(node -> terminal){
...
void Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::take_step(bool verbose){
    Actions::Action* chosen_action = generate_action(verbose);
    active_game.applyAction(chosen_action);
    if(verbose){
        DEBUG_MSG(std::endl << "[KSample_Precondition_UCTMaxChildAgent_SmartRollout::take_step()] " << active_game.get_board_ptr() -> active_player().role.name << ": " << chosen_action -> repr() << std::endl);
    }
...
void Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::reset(){
    tree_depths.clear();
...
std::vector<std::string> Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::get_keys(){
    return {
        "AvgTreeDepth",
        "StdTreeDepth",
        "MaxTreeDepth",
...
std::vector<double> Agents::KSample_Precondition_UCTMaxChildAgent_SmartRollout::get_values(){
    // Avg tree depth
    double depth_mean=0;
