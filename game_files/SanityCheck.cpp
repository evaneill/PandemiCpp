...
void SanityCheck::CheckBoard(Board::Board& active_board,bool verbose){
    ...
    std::cout << std::endl << "[SANITYCHECK] Checking disease counts on each city..." << std::endl;
    for(int col=0;col<4;col++){
        for(int city=0;city<Map::CITIES.size();city++){
            if(active_board.get_disease_count()[col][city]<0 || active_board.get_disease_count()[col][city]>3){
                if(verbose){
                    std::cout << "[SANITYCHECK] ... " << Map::CITIES[city].name << " has " << active_board.get_disease_count()[col][city] << " " << Map::COLORS[col] << " cubes on it! that's bad." << std::endl;
                }
    ...
    std::cout << std::endl << "[SANITYCHECK] Checking infection card drawn counter is 0 when not infect step..." << std::endl;
    if(active_board.get_turn_action()<=4){
        if(active_board.get_infect_cards_drawn()>0){
            if(verbose){
                std::cout << "[SANITYCHECK] ... but on turn-action " << active_board.get_turn_action() << " the counter is >0!" << std::endl;
            }
            active_board.broken()=true;
    ...
    if(verbose){
        std::cout << "[SANITYCHECK] done!" << std::endl;
    }

    // make sure player card draw is 0 outside of that stage
    std::cout << std::endl << "[SANITYCHECK] Checking player cards drawn counter is 0 when not player draw step..." << std::endl;
    ...
    std::cout << std::endl << "[SANITYCHECK] Checking player hands for duplicate cards..." << std::endl;
            ...
            for(int c=0;c<(p.hand.size()-1);c++){
                for(int k=c+1;k<p.hand.size();k++){
                    if(p.hand[c].index==p.hand[k].index){
                        if(verbose){
                            std::cout << "[SANITYCHECK] ... player " << p.role.name << " has two of " << p.hand[c].name << "!" << std::endl;
                        }
                        active_board.broken()=true;
    ...
    std::cout << std::endl <<  "[SANITYCHECK] Checking all players have <=8 cards..." << std::endl;
    for(Players::Player& p: active_board.get_players()){
        if(p.handsize()>8){
            if(verbose){
    ...
    std::cout << std::endl <<  "[SANITYCHECK] Checking that there are <=difficulty epidemic cards drawn" << std::endl;
    if(active_board.get_epidemic_count()<0 || active_board.get_epidemic_count()>active_board.get_difficulty()){
        if(verbose){
            std::cout << "[SANITYCHECK] ... but there are " << active_board.get_epidemic_count() << " epidemics, even though difficulty is " << active_board.get_difficulty() << std::endl;
        }
        active_board.broken()=true;
